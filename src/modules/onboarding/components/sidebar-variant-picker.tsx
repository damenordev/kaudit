...
import { setSidebarVariantAction } from '@/core/ui/navigation/sidebar.actions'
import { cn } from '@/core/utils/cn.utils'
import type { TSidebarVariant } from '@/core/ui/navigation/sidebar.constants'

interface ISidebarVariantPickerProps {
  initialVariant: TSidebarVariant
}

const OPTIONS: { value: TSidebarVariant }[] = [{ value: 'sidebar' }, { value: 'inset' }, { value: 'floating' }]
...
export function SidebarVariantPicker({ initialVariant }: ISidebarVariantPickerProps) {
  const t = useTranslations('onboarding.appearance')
  const [variant, setVariant] = useState<TSidebarVariant>(initialVariant)
  const [isPending, startTransition] = useTransition()

  const handleSelect = (value: TSidebarVariant) => {
    setVariant(value)
    startTransition(() => {
      setSidebarVariantAction(value)
    })
  }
