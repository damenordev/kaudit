...
export default async function SignInPage() {
  ...
  return (
    <div className="container relative min-h-screen flex-col items-center justify-center grid lg:max-w-none lg:grid-cols-2 lg:px-0 bg-background">
      <div className="relative hidden h-full flex-col bg-sidebar p-10 text-sidebar-foreground lg:flex border-r border-sidebar-border">
        <div className="absolute inset-0 bg-sidebar" />
        {/* Subtle mesh/radial effect using theme-primary with very low opacity */}
        <div className="absolute inset-0 bg-[radial-gradient(ellipse_at_top_left,var(--primary),transparent)] opacity-[0.03]" />
        ...
        <div className="relative z-20 mt-auto">
          <blockquote className="space-y-4">
            <p className="text-2xl font-medium leading-normal tracking-tight">&ldquo;{t('signIn.quote')}&rdquo;</p>
      ...
      <div className="p-4 lg:p-8 flex items-center justify-center h-full">
        <div className="mx-auto flex w-full flex-col justify-center space-y-8 sm:w-[350px]">
          <div className="flex flex-col space-y-3 text-center">
            <h1 className="text-3xl font-bold tracking-tight text-foreground">{t('signIn.title')}</h1>
            <p className="text-sm text-muted-foreground leading-relaxed px-2">{t('signIn.description')}</p>
          </div>
