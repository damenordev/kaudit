...
export async function InstallSuccess() {
  ...
  return (
    <div className="w-full max-w-lg mx-auto p-10 rounded-3xl border border-border/40 bg-muted/10 shadow-xs text-center space-y-8 animate-in fade-in zoom-in-95 duration-500">
      ...
      <div className="space-y-3">
        <h2 className="text-2xl font-semibold tracking-tight text-foreground">{t('success.title')}</h2>
        <p className="text-[15px] text-muted-foreground leading-relaxed px-2">{t('success.description')}</p>
      </div>
      ...
      <div className="pt-2">
        <Button
          asChild
          size="lg"
          className="w-full h-12 rounded-xl text-base font-medium shadow-sm transition-transform active:scale-95 group/btn"
        >
